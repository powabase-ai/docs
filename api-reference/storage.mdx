...
The Anon Key combined with a user's access token in `Authorization` is the right pattern for browser-direct uploads. Service Role in both is for server-side admin operations.
