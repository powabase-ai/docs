...
Remove a source from a knowledge base. Cascades to delete the source's chunks, so the source no longer contributes to retrieval in this KB. The underlying Source itself is not deleted — only its link to this KB and the chunks indexed for it. Returns 204 on success and 404 if no `indexed_sources` row matches the given pair.
