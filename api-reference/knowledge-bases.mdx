...
Knowledge bases provide semantic search over your document content. They store chunked and embedded text from one or more sources for retrieval-augmented generation (RAG). When you add a source, the platform chunks the source's page texts, generates vector embeddings, and stores them for similarity search.

## Common Patterns

Create a knowledge base, add one or more sources to trigger indexing, then query with the search endpoint. Check indexing status by fetching the knowledge base details. Reindex when you change chunking parameters or want to re-process sources.
...
Dispatch a one-shot BM25 rebuild for this KB. Re-tokenizes the entire item table for the KB's strategy (chunks / full_documents / graph_index_nodes) and writes a fresh BM25 index, replacing whatever was there. An operator path: most KBs never need it, but it's useful after changing `retrieval_config` tuning knobs or recovering from a partial index.
...
Per-request overrides for retrieval tuning are also accepted: `vector_weight` (hybrid only), `context_mode` (e.g. surrounding-chunks context), `ts_language` (full-text language for stemming), reranker fields. These override the KB's stored `retrieval_config` for this one request only.
...
Returns the platform's KB-creation defaults so a UI or self-service KB-creation flow can present valid options without hardcoding them. The Studio's KB-create wizard uses this exact endpoint.
