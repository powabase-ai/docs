...
List all configured provider keys. Each entry includes `id`, `provider`, `masked_key` (the only representation of the secret the API returns), `is_valid`, `last_validated_at`, `created_at`, and `updated_at`.
