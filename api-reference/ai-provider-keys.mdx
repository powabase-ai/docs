...
The Studio's LLM Provider Keys settings page uses this to render "AI-on-us active" vs "BYOK required" badges per provider. It's also the right pre-flight check before pointing an agent at a model whose provider you haven't BYOK'd yet — if the provider is `platform_supported`, the model just works; otherwise the agent run will return `402 provider_key_decrypt_failed` until you upsert your own key.
