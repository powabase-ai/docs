...
<Note>
The body MUST use the key `config_override`. Passing `config` (or any other top-level key) is a silent no-op — the route returns 200 with the unchanged assignment because the handler only inspects `config_override`. For database tools, `config_override.schemas` is validated: schemas must be a dict, schema names must match `^[a-zA-Z_][a-zA-Z0-9_]{0,63}$`, system schemas (`ai`, `auth`, `storage`, `pg_*`, etc.) are rejected, and each value must be a list of valid table names.
</Note>
