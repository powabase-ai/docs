...
| `max_context_tokens` | int | 1000–128000. Honored only when the run's search tool resolves to exactly one KB (attached + runtime combined) |
...
- **Combinable** with the context fields above (it adds a tool, not injected context). Combined with the preload `knowledge_bases` field, both retrievals run and are billed; no dedup.
