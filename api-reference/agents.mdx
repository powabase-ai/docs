...
- **Database tools (`database_query`, `database_write`)**: `config_override.schemas: { <schema>: [<table>, ...] }` is the table list. `database_write` rejects tables outside it; `database_query` only sets `search_path` from it, so a schema-qualified `SELECT` on other tables isn't blocked (see [Agents & Tools](/concepts/agents-tools#builtin-tools)). Schemas must be a dict, schema names must match `^[a-zA-Z_][a-zA-Z0-9_]{0,63}$`, system schemas (`ai`, `auth`, `storage`, `pg_*`, etc.) are rejected, and each value must be a list of valid table names.
