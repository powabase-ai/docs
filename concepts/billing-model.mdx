...
Powabase bills in **credits**. Every billable operation in the platform debits credits from the project's organization: running an agent, indexing a source, executing a workflow block, searching a knowledge base. Free-plan organizations get a one-time sign-up credit that does not refill; after that it's pay as you go. Paid plans add a monthly credit grant equal to the subscription price, and unused credits roll over. If you run out, the platform returns `402` until you top up or upgrade.
...
Powabase has a free plan and paid plans (Self Serve and Scale; see the pricing page on powabase.ai):

- **`free`:** a one-time sign-up credit, with no monthly refill. Once it's spent, top up or upgrade. Hard cap: balance must be >= estimated cost or the request returns `402`.
- **Paid plans (`self-serve`, `scale`):** each paid billing cycle grants credits equal to the subscription price, and unused credits roll over. The pre-dispatch check uses a "soft cap": the request is refused when the estimated cost exceeds the balance plus a configurable grace amount (`BILLING_PAID_TIER_SOFT_CAP_GRACE_CREDITS`). The grace defaults to `0`, so by default the check behaves the same as the free-plan hard cap.

The plan tier is propagated via the `BILLING_PLAN_TIER` env var on the project-service pod, set from the organization's plan when the project is provisioned, and defaults to `"free"`.
...
- **`renews_at`:** the first of the next UTC month. Free plans don't refill, so for a free-plan org this is not a date when credits come back.

The frontend Studio renders this as a "You're out of credits" banner. Your own clients should do the same: when a `402 insufficient_credits` comes back, the right user-facing response is "you're out of credits, top up or upgrade" rather than retrying.
...
|---|---|
| `402 insufficient_credits` | Tell the user they're out of credits; don't retry. Suggest top-up or upgrade. |
