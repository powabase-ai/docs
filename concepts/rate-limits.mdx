...
Powabase's only quantitative rate limit on the AI surface today applies to workflow execution endpoints. The rest of the API — agent runs, knowledge search, source CRUD, etc. — has no Kong-level or in-process limiter; abuse protection there comes from credit-based billing (you can't run 10,000 agents per second because you'd exhaust your credits).
