...
description: "The shared patterns across all of Powabase's HTTP APIs — header conventions, naming, error envelopes, pagination, response shapes. Once you know these, every endpoint is easier to read."
icon: "ruler"
---
...
The pattern was that PATCH is partial-update (body keys not present aren't changed) and PUT is full-replace, but in practice the platform's PUT endpoints also work as partial updates. The choice is historical, not principled.
