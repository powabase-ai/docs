...
The platform uses a two-tier architecture. A single shared Control Plane manages organizations, projects, users, and authentication. It provisions a Data Plane for each project, with its own Postgres database, auth service, Storage API, Realtime and AI service. On Powabase Cloud, a few pieces are shared across projects rather than per project: the API gateway, the S3 bucket behind Storage, and the Redis the AI workers use as their task queue.

<Frame caption="Two tiers: a shared control plane for management, and per-project data planes for runtime traffic">
  <img src="/diagrams/architecture-overview.svg" alt="Architecture diagram: Browser → Next.js Frontend → Control Plane API for management; runtime requests go Browser → Kong gateway (shared across projects on Cloud, routing by hostname) → Per-Project Stack (GoTrue Auth, PostgREST, Project Service, Postgres with pgvector, Storage, Realtime). The control plane provisions and manages projects but is not in the data-plane path for runtime traffic." />
</Frame>

## Per-Project Isolation

Each project gets its own infrastructure stack in its own Kubernetes namespace (`project-{ref}`). One project's data, users, and configuration are separate from another's; projects can't see each other's databases, files, or API keys. Isolation is enforced at three layers:
...
- **Storage:** all projects share one S3 bucket. Each project's Storage API is configured with its own prefix (`projects/<ref>/`) and only reads and writes object paths under it. That boundary is enforced by the Storage API (application layer), not by a separate bucket or separate IAM credentials. Your Storage buckets (`storage.buckets`) are rows in your project's own database, so bucket names and policies are per project.
...
When you make an API call to your project URL (`<ref>.p.powabase.ai`), the request hits the Kong gateway directly; the control plane is **not** in the data-plane path. On Powabase Cloud, one shared Kong deployment (`kong-projects`, in the `shared-services` namespace) serves every project: it matches the project hostname and routes `/api/*` paths to the Project Service (AI features), `/auth/v1/*` to GoTrue (authentication), `/rest/v1/*` to PostgREST (direct database access), `/storage/v1/*` to the Storage API, and `/realtime/v1/*` to the Realtime service.
...
| `ai` | Platform | Sources, knowledge bases, chunks, embeddings, agents, sessions, runs, workflows | No (backend-only; use the typed `/api/*`) |
| `storage` | Storage API | File metadata, bucket configuration | Yes |
| `auth` | GoTrue | User accounts, sessions, tokens | No (use the Auth API) |

PostgREST is configured to expose `public`, `storage`, and `graphql_public` schemas. The `ai` schema is backend-only: it isn't exposed through PostgREST, and only the project service (using the service role) reads and writes it. Reach AI resources through the typed `/api/*` endpoints.
