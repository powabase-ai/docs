...
description: "The footguns Powabase users hit most: silent failures, wrong field placements, the backend-only ai schema, the differences between agentic and database webhooks. With concrete fixes."
...
### "Any signed-in user can reach every other user's agents"

Not through the database: the `ai` schema is backend-only, and `authenticated` and `anon` have no grants or policies on it. But the typed `/api/*` surface accepts any signed-in user's JWT, and it only enforces ownership on sessions and runs. Agents, knowledge bases, workflows and knowledge-base search are project-wide. For multi-user projects, call `/api/*` from your own backend with the Service Role key and enforce access there. See [The ai schema](/concepts/ai-schema-postgrest).

### Querying `ai.*` via PostgREST fails, even with `Accept-Profile: ai`

The `ai` schema isn't exposed through PostgREST (it's not in `PGRST_DB_SCHEMAS`), whichever key you send. Reach AI resources through the typed `/api/*` endpoints instead.
