...
export const agentsToolsConcept: ConceptPage = {
  id: "agents-tools",
  title: "Agents & Tools",
  ...
  content: [
    {
      type: "heading", level: 2, text: "What is an Agent?", id: "what-is-agent",
    },
    {
      type: "prose",
      text: "An Agent wraps an LLM with a system prompt, temperature, a set of tools, and optional knowledge bases. When you send a message, the agent enters a ReAct (Reason + Act) loop: the LLM decides whether it needs to call a tool, executes the tool if needed, observes the result, and either calls another tool or generates a final response. This loop continues for up to 25 steps (configurable per run). On the final step, tools are withheld so the LLM is forced to produce a text response.",
    },
    {
      type: "heading", level: 2, text: "The ReAct Loop", id: "react-loop",
    },
    {
      type: "diagram",
      svgId: "agent-react-loop",
      caption: "Agent ReAct loop: reason, act, observe, respond",
      alt: "ReAct loop diagram: User Message → LLM (Reason) → Tool Call? If yes → Execute Tool → Tool Result → back to LLM. If no → Generate Response → Stream to User.",
    },
    {
      type: "prose",
    ...
    {
      type: "heading", level: 3, text: "Context Management", id: "context-management",
    },
    {
      type: "prose",
      text: "The agent automatically manages its context window. Before each LLM call, it estimates the token count and, if nearing the model's context limit, triggers proactive compaction: first pruning old tool results (replacing them with placeholders while keeping the last 3 user turns), then if still over the limit, summarizing the older conversation using a lightweight LLM (gpt-4.1-nano). If the LLM returns a prompt_too_long error, the agent retries with compaction. If the LLM output is truncated, the agent injects a \"continue where you left off\" message and retries up to 3 times.",
    },

    // ─── TOOLS ─────────────────────────────────────────────────────

    {
      type: "heading", level: 2, text: "Tool Types", id: "tool-types",
    },
    {
      type: "prose",
      text: "Agents can use three categories of tools: builtin tools provided by the platform, custom tools that call your HTTP endpoints, and MCP server tools discovered at runtime.",
    },

    {
      type: "heading", level: 3, text: "Builtin Tools", id: "builtin-tools",
    },
    {
      type: "prose",
      text: "The platform provides eight builtin tools. Assign them to an agent by name via POST /api/agents/{id}/tools. The database tools include schema-level access control — when assigning them, you configure which schemas and tables the agent can access.",
    },
    {
      type: "table",
      headers: ["Tool", "Description", "Constraints"],
      rows: [
      ...
      cards: [
        {
          ...
          description: "Create an agent, assign tools, and start chatting.",
          icon: "Bot",
          target: { type: "guide", guideId: "build-agent" },
        },
        {
          title: "Advanced Agent Config",
          description: "Add MCP servers, hooks, and approval flows.",
          icon: "Wrench",
          target: { type: "guide", guideId: "advanced-agent-config" },
        },
        {
