...
export const knowledgeBasesIndexingConcept: ConceptPage = {
  id: "knowledge-bases-indexing",
  ...
  content: [
    {
      type: "heading", level: 2, text: "What is a Knowledge Base?", id: "what-is-kb",
    },
    {
      type: "prose",
      text: "A Knowledge Base (KB) is a container that holds one or more sources and makes their content searchable. When you add a source to a KB, the platform processes the extracted text using the configured indexing strategy and stores the results for retrieval. The indexing strategy determines how content is structured — from simple chunking to hierarchical document trees to structured JSON extraction. The retrieval strategy determines how queries find relevant content — from fast vector similarity to LLM-driven reasoning over document structure.",
    },
    {
      type: "heading", level: 2, text: "Indexing Pipeline", id: "indexing-pipeline",
    },
    {
      type: "prose",
      text: "The indexing pipeline runs automatically when you add a source to a knowledge base. What happens during indexing depends on the strategy: ChunkEmbed splits text and generates embeddings, PageIndex builds a hierarchical tree with LLM-generated summaries, GraphIndex extends PageIndex with cross-reference detection and node embeddings, and Doc2JSON extracts structured fields. You can reindex at any time to reprocess with different settings.",
    },
    {
      type: "diagram",
      svgId: "indexing-pipeline",
      caption: "Indexing pipeline: from source text to searchable knowledge",
      alt: "Indexing pipeline: Source page texts → Indexing Strategy (ChunkEmbed, PageIndex, GraphIndex, or Doc2JSON) → Storage (pgvector chunks, tree nodes, or structured JSON).",
    },

    // ─── INDEXING STRATEGIES ───────────────────────────────────────

    {
      type: "heading", level: 2, text: "Indexing Strategies", id: "indexing-strategies",
    },
    {
      type: "prose",
      text: "The indexing strategy controls how source content is processed and stored. Each strategy produces different data structures, supports different retrieval methods, and has different cost profiles. You set the strategy when creating or updating a knowledge base via the indexing_config field.",
    },
    {
      type: "table",
      headers: ["Strategy", "Best For", "Cost", "Indexing Speed", "Retrieval Speed", "Compatible Retrieval"],
      rows: [
    ...
    // --- ChunkEmbed ---
    {
      type: "heading", level: 3, text: "ChunkEmbed (Default)", id: "chunk-embed",
    },
    {
      type: "prose",
      text: "The standard RAG approach. Text is split into overlapping chunks using a configurable chunking strategy, each chunk is embedded into a vector, and the vectors are stored in pgvector for similarity search. A BM25 sparse index is also built over the chunk text, enabling keyword-based full-text search alongside vector search. This is the fastest and cheapest indexing strategy — no LLM calls are needed, only an embedding API call.",
    },
    {
      type: "prose",
      text: "ChunkEmbed pairs with vector search, hybrid search, or full-text search for retrieval. Hybrid search (which runs both vector and BM25 in parallel and fuses results) is the recommended default for production RAG applications.",
    },
    {
      type: "heading", level: 3, text: "Chunking Strategies", id: "chunking-strategies",
    },
    {
      type: "prose",
      text: "ChunkEmbed supports three chunking strategies that control how text is split into searchable units. The chunking strategy is independent of the embedding model.",
    },
    {
      type: "table",
      headers: ["Chunker", "How It Works", "Best For"],
      rows: [
    ...
    {
      type: "prose",
      text: "Chunk size and overlap control the tradeoff between precision and context. Smaller chunks (500–1000 tokens) give more precise retrieval but may lose surrounding context. Larger chunks (2000–4000 tokens) preserve context but can dilute relevance. The defaults (2000 tokens, 50 token overlap) work well for most use cases.",
    },
    {
      type: "code",
      snippets: {
        python: `# Create a KB with ChunkEmbed (the default strategy)
...
kb = response.json()`,
        typescript: `const res = await fetch(\`\${BASE_URL}/api/knowledge-bases\`, {
  method: "POST",
  headers,
  body: JSON.stringify({
    name: "Product Docs",
...
response = requests.post(
  ...
  -d '{
    ...
    {
      type: "heading", level: 2, text: "Retrieval Strategies", id: "retrieval-strategies",
    },
    {
      type: "prose",
      text: "The retrieval strategy controls how queries find relevant content within a knowledge base. You set the retrieval method when creating a KB or when calling the search endpoint. The right choice depends on your indexing strategy, query patterns, latency requirements, and budget.",
    },
    {
      type: "table",
...
response = requests.post(
  ...
  -d '{
    ...
    },
    {
      type: "callout",
      variant: "info",
      title: "Reranker API keys are platform-managed",
      text: "Reranker API keys (Cohere, Jina, Voyage) are configured at the platform level by your administrator, not per-organization. If reranking returns errors, contact your platform admin to verify the reranker provider key is configured.",
    },

    // ─── EMBEDDING MODELS ──────────────────────────────────────────

    {
      type: "heading", level: 2, text: "Embedding Models", id: "embedding-models",
    },
    {
      type: "prose",
      text: "Embeddings convert text into high-dimensional vectors that capture semantic meaning. The platform uses OpenAI's text-embedding-3-small by default (1536 dimensions). All chunks in a knowledge base must use the same embedding model — if you change the model, you must reindex. Embedding calls are batched at up to 250,000 tokens per API call for efficiency. The platform supports embedding models from multiple providers via LiteLLM — select your preferred model in Settings > Knowledge Indexing.",
    },
    {
      type: "table",
      headers: ["Model", "Provider", "Dimensions", "Tradeoff"],
      rows: [
    ...
    {
      type: "heading", level: 2, text: "Searching a Knowledge Base", id: "search",
    },
    {
      type: "prose",
      text: "Once indexed, you can search a knowledge base with any natural language query. The search uses whichever retrieval method was configured on the KB, or you can override it per-request. Results include the matched text, relevance scores, and source metadata.",
    },
    {
      type: "code",
      snippets: {
...
results.forEach((chunk: any) => {
  ...
  -d '{"query": "How do I reset my password?", "top_k": 5}'`,
    ...
    {
      type: "heading", level: 2, text: "Reindexing", id: "reindexing",
    },
    {
      type: "callout",
      variant: "warning",
      title: "Reindexing replaces all indexed content",
      text: "When you reindex a knowledge base, all existing chunks, tree nodes, or extracted JSON are deleted and recreated from scratch. The KB remains searchable during reindexing but results may be incomplete until it finishes. For large KBs with PageIndex or GraphIndex, reindexing can take significant time and LLM tokens.",
    },

    // ─── RECOMMENDED CONFIGURATIONS ────────────────────────────────

    {
      type: "heading", level: 2, text: "Recommended Configurations", id: "recommended-configs",
    },
    {
      type: "table",
      headers: ["Use Case", "Indexing", "Retrieval", "Notes"],
      rows: [
      ...
      cards: [
        {
          ...
          description: "Step-by-step guide to creating and indexing a KB.",
          icon: "Database",
          target: { type: "guide", guideId: "create-knowledge-base" },
        },
        {
          title: "Agents & Tools",
          description: "Attach a KB to an agent for RAG-powered conversations.",
          icon: "Bot",
