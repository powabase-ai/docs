...
<Tip>
The retrieval features — [reranking](/concepts/knowledge-bases-indexing#reranking), [query enrichment](/concepts/knowledge-bases-indexing#query-enrichment), and [multimodal retrieval](/concepts/knowledge-bases-indexing#multimodal-retrieval) (`context_mode: "image"`) — all live inside `retrieval_config`, so you can turn them on here at creation or add them later with `PATCH /api/knowledge-bases/{id}` (no reindex required).
</Tip>
