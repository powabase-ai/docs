...
Beyond basic tools and knowledge bases, agents take three further configuration options. MCP (Model Context Protocol) servers connect external tools. Hooks fire webhooks on agent lifecycle events. And an approval flow pauses execution until a human signs off on sensitive tool calls. This guide walks through each one.
...
Add an HTTP webhook hook that fires before each tool call. Hooks are useful for logging tool usage, enforcing policies, or notifying external systems.
