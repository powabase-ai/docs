...
- **Permissive vs restrictive policies.** All policies are `PERMISSIVE` by default — they OR together. `RESTRICTIVE` policies AND together with the result. Useful when you want to layer a "no row may be deleted on Sundays" check on top of existing permissive policies without rewriting them.

- **Functions in policy expressions.** Postgres caches policy expression results per row per query. A `SELECT 1 FROM members WHERE ...` subquery is fine; an HTTP call from inside a policy (via `pg_net`) is not — it'll run thousands of times. Keep policy expressions cheap and deterministic.
