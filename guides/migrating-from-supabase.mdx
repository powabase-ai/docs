...
The most novel thing about Powabase is the `ai` schema. It's where the AI surface's state lives: sources, knowledge_bases, agents, workflows, and the rest. You can query it via PostgREST under RLS (35+ tables exposed); see [Querying the ai schema](/concepts/ai-schema-postgrest).
