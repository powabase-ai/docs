...
For a side-by-side of what Powabase adds on top of Supabase, see [Powabase as a Supabase alternative](https://powabase.ai/supabase-alternative/) and the step-by-step [migration overview](https://powabase.ai/migrate-from-supabase/) on the website. For the BaaS surface in general, see the relevant per-area pages. For the agentic API (the Powabase-specific addition), start from the [Auth & Connection](/guides/auth-connection) guide and follow the links into the AI surface.
...
pg_graphql is preloaded and the `graphql_public` schema is in PostgREST's exposed schemas. But there's no dedicated `/graphql/v1/*` Kong route, so call GraphQL through PostgREST RPC:
...
The most novel thing about Powabase is the `ai` schema. It's where the AI surface's state lives: sources, knowledge_bases, agents, workflows, and the rest. It's backend-only: not exposed via PostgREST. You reach it through the typed `/api/*` endpoints; see [The ai schema](/concepts/ai-schema-postgrest).
...
- **Four retrieval methods:** vector_search, full_text (BM25), hybrid, tree_search (for the page_index strategy).
...
4. **Gate the AI surface in your backend** if your app exposes it to clients: `/api/*` accepts any signed-in user's JWT and only enforces ownership on sessions and runs.
5. **Replace billing assumptions:** soft limits become a hard 402 when credits run out (top up or upgrade; the free plan doesn't refill).
