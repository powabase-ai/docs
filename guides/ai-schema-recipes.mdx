...
`SECURITY DEFINER` lets the function run with the owner's permissions rather than the caller's — useful here because the aggregate doesn't need per-user filtering and you want consistent results regardless of which key calls it. Set `search_path = ai` explicitly to avoid the [search-path SECURITY DEFINER pitfall](https://www.postgresql.org/docs/current/sql-createfunction.html#SQL-CREATEFUNCTION-SECURITY).
