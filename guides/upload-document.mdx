...
Poll the source until extraction_status reaches a terminal state: extracted (success), attention_required (partial: some pages failed but the source is still indexable), failed, or cancelled. Small documents typically take a few seconds.
