...
Powabase is available for self-hosting as part of the enterprise offering. The enterprise deployment is not a publicly self-serve product (for a single project, see the [open-source stack](https://github.com/powabase-ai/powabase) and the [self-hosted Supabase overview](https://powabase.ai/self-hosted-supabase/)). The deployment is Kubernetes-based, the operations posture is "you run it, we support you," and you engage directly with the platform team rather than through docs-driven self-service.
...
- **`project-stack`**: the per-project resources (one Postgres StatefulSet, one GoTrue deployment, one Storage API deployment, one Realtime deployment, one project-api deployment). Provisioned once per project in its own namespace.
- **`shared-services`**: the cluster-wide resources (PgBouncer, imgproxy, the wildcard ALB ingress, monitoring stack, and the single `kong-projects` gateway that routes every project's traffic by `Host` header). Provisioned once per cluster.
...
The Helm charts target AWS (EKS): they use IAM roles for service accounts, External Secrets backed by AWS Secrets Manager, S3, and the AWS Load Balancer Controller for the wildcard ingress. Other environments are possible with chart adaptations; contact us to discuss yours.
...
A single-project stack is open source under Apache 2.0 at [github.com/powabase-ai/powabase](https://github.com/powabase-ai/powabase): one `docker compose up` runs Postgres, Auth, Storage, REST and the Powabase AI service for one project, with no control plane. The enterprise Helm charts and the multi-project platform code (control plane, provisioner, billing) aren't open-sourced. If that's a hard requirement for your use case, raise it during sales; the platform's posture here may evolve.
