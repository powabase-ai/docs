...
You can fire an HTTP request from inside Postgres when a row in a table changes. The mechanism is a trigger function (`supabase_functions.http_request()`) plus `pg_net` (the async HTTP extension) — both preloaded in every Powabase project. This is the pattern for "tell our internal API every time a user signs up," "post to Slack on order creation," or "invalidate a cache when this row updates."
