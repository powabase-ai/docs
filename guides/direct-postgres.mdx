...
| `42P01` | undefined_table | Table doesn't exist (or you forgot to schema-qualify it or set `search_path`, e.g. for `ai.*`) |
...
| Reports on AI resources (agents, sources, KB status) | Typed `/api/*` (`ai.*` isn't exposed through PostgREST) |
