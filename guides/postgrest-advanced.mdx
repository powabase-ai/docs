...
For the framing, see [PostgREST reference](/api-reference/postgrest). These patterns apply to your own schemas (`public`, `storage`); the `ai` schema isn't exposed through PostgREST, so for AI resources see [AI resource recipes](/guides/ai-schema-recipes).
...
The ai schema isn't exposed through PostgREST; these recipes use the typed /api/* endpoints instead.
</Card>
